...
export namespace explorer {
	...
	export class FolderNavigation {
	    prevFolder?: FolderInfo;
	    nextFolder?: FolderInfo;
	    parentPath: string;
	    currentIndex: number;
	    totalFolders: number;
	    ...
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.prevFolder = this.convertValues(source["prevFolder"], FolderInfo);
	        this.nextFolder = this.convertValues(source["nextFolder"], FolderInfo);
...
export namespace persistence {
	...
	export class AlgorithmDownloadConfig {
	    maxParallelChapters: number;
	    maxParallelImages: number;
	
	    static createFrom(source: any = {}) {
	        return new AlgorithmDownloadConfig(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.maxParallelChapters = source["maxParallelChapters"];
	        this.maxParallelImages = source["maxParallelImages"];
	    }
	}
	export class ChapterInfo {
	    path: string;
