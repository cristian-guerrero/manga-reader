...
export function ColorizerPage() {
    ...
    OnFileDrop(async (_x, _y, paths) => {
      ...
      try {
        ...
        for (const p of paths) {
          ...
          try {
            const entries = await AppBackend.ExploreFolder(resolved, '', 'asc');
  ...
  useEffect(() => {
    ...
    const loadFolderImages = async () => {
      try {
        setIsLoadingImages(true);
        const entries = await AppBackend.ExploreFolder(folderPath, '', 'asc');
        const newImages: ImageFile[] = [];
        for (const entry of entries) {
          if (!entry.isDirectory && entry.hasImages) {
            newImages.push({
  ...
  const handleSelectFolder = useCallback(async () => {
    try {
      ...
      if (path) {
        setIsLoadingImages(true);
        try {
          const entries = await AppBackend.ExploreFolder(path, '', 'asc');
          const newImages: ImageFile[] = [];
          for (const entry of entries) {
            if (!entry.isDirectory && entry.hasImages) {
              newImages.push({
