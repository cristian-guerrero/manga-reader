...
export function ColorizerPage() {
    ...
    useEffect(() => {
        ...
        console.log('[Colorizer] colorizedCache keys:', Object.keys(colorizedCache));
        console.log('[Colorizer] currentColorizedImage length:', currentColorizedImage?.length ?? null);
    ...
    const handleColorize = useCallback(async () => {
        ...
        try {
            ...
            console.log('[Colorize] Result:', JSON.stringify(result).substring(0, 500));
            console.log('[Colorize] success:', result.success, 'output_base64 length:', result.output_base64?.length);
                            ...
                            droppedImages.map((img, idx) => (
                                <button
                                    ...
                                    className={`w-full px-3 py-2 rounded-lg text-sm text-left truncate transition-all ${
                                        currentImage === img.path ? 'bg-accent/20 border-accent' : 'hover:bg-surface-hover'
                                    }`}
                                    style={{
                                        backgroundColor: currentImage === img.path ? 'var(--color-accent)' : 'rgba(0,0,0,0)',
                                        color: currentImage === img.path ? 'white' : 'var(--color-text-primary)',
                                        border: currentImage === img.path ? '1px solid var(--color-accent)' : '1px solid transparent',
                                    }}
                                >
                                    {img.name}
                                </button>
                    ...
                    <div className="flex-1 min-h-0 flex items-center justify-center p-6 relative">
                        {currentColorizedImage ? (
                            ...
                            <div className="text-center">
                                <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" style={{ color: 'var(--color-text-muted)', margin: '0 auto' }}>
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                ...
                                className="px-6 py-2 rounded-lg text-sm font-semibold transition-all hover:scale-[1.02] active:scale-[0.98]"
                                style={{
                                    backgroundColor: currentColorizedImage ? 'var(--color-success)' : 'var(--color-surface-tertiary)',
                                    color: currentColorizedImage ? 'white' : 'var(--color-text-muted)',
