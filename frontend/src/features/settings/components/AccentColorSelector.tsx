...
export function AccentColorSelector({
  ...
  return (
    <div className="flex flex-wrap gap-3">
      {/* Default / Reset */}
      <Tooltip
        content={t("settings.accentDefault", "Default Theme Accent")}
        placement="bottom"
      >
        <button
          ...
          className={`
            ...
            ${!currentAccentColor ? "ring-2 ring-offset-2 scale-110" : "hover:scale-105"}
          `}
          style={{
            background:
      ...
      {ACCENT_COLORS.filter(
        (c: { id: string; color: string; name: string }) => c.id !== "default",
      ).map((color: { id: string; color: string; name: string }) => (
      ...
      <div className="relative group">
        ...
        >
          <Palette
            className="w-4 h-4 text-text-muted group-hover:text-accent"
