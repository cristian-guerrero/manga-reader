  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 flex-1 min-w-0">
        ...
        <div className="flex flex-col min-w-0">
          <Tooltip content={folderName}>
            <span
              className="text-sm font-medium truncate max-w-xs w-full min-w-0"
          ...
          {chapterNav && (
            <Tooltip
              content={`${chapterNav.seriesName} • Cap. ${(chapterNav.chapterIndex ?? 0) + 1}/${chapterNav.totalChapters}`}
            >
              <span
                className="text-xs truncate max-w-xs w-full min-w-0"
                style={{ color: "var(--color-text-secondary)" }}
              >
                {chapterNav.seriesName} • Cap.{" "}
                {(chapterNav.chapterIndex ?? 0) + 1}/{chapterNav.totalChapters}
              </span>
            </Tooltip>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 flex-1 justify-end">
        ...
        <div className="relative z-20">
          <Tooltip
            content={t("viewer.thumbnails") || "Thumbnails"}
            placement="bottom"
          >
            <button
        ...
        <div className="relative z-20">
          <Tooltip
            content={
              mode === "vertical" ? t("viewer.lateral") : t("viewer.vertical")
            }
            placement="bottom"
          >
            <button
        ...
        <div className="relative z-20">
          <Tooltip
            content={t("viewer.goToStart") || "Go to Start"}
            placement="bottom"
          >
            <button
              onClick={onGoToStart}
