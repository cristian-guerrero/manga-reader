...
const buildBreadcrumbSegments = (
  ...
  const baseFolder = baseFolders.find((bf) => {
    ...
    return (
      normalizedPath.startsWith(normalizedBfPath + "/") ||
      normalizedPath === normalizedBfPath
    );
  });
  ...
  if (baseFolder) {
    ...
    const relativePath = pathNormalized.substring(
      basePathNormalized.length + 1,
    );
              ...
              {isClickable ? (
                <Tooltip content={segment.path || segment.name}>
                  <button
                    ...
                    onAuxClick={(e) =>
                      onAuxClick?.(e, segment.path, segment.name)
                    }
