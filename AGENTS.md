...
- `ApplyUpdate()` validates that the pending version is newer than the current version before applying; stale files are cleaned up automatically. If no version file exists, the update is rejected and cleaned up.
